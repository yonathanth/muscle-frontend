  ...
  const fetchNotifications = async () => {
    try {
      ...
      const [userRes, broadcastRes] = await Promise.all([
        fetch(`${NEXT_PUBLIC_API_BASE_URL}/api/members/${user?.id}/`, {
          cache: "no-store",
        }),
        fetch(`${NEXT_PUBLIC_API_BASE_URL}/api/broadcast/`, {
          cache: "no-store",
        }),
      ]);
      ...
      const userNotifications: NotificationType[] =
        userData?.data?.user?.notifications || [];
      const broadcastNotifications: NotificationType[] =
            ...
            >
              {isLoading ? (
                ...
                <ul
                  ...
                  style={{
                    scrollbarWidth: "thin",
                    scrollbarColor: "#871818 #1E293B",
                  }}
                >
                  {notifications?.map((notification) => {
                    const updatedDate = new Date(notification.updatedAt); // Convert to Date object
                    const formattedDate =
                      updatedDate.toLocaleDateString("en-GB"); // Format to dd-mm-yyyy
                    return (
                      <li
